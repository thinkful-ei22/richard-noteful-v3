...
describe('Noteful Api resource', () => {
    ...
    it('should GET tag with given id', () => {
      ...
      return Tag.findOne()
        ...
        .then(result => {
          ...
          expect(result.body).to.have.all.keys('id', 'name', 'createdAt', 'updatedAt');
  ...
  describe('PUT endpoints', () => {
    it('should update a tag with given id', () => {
      ...
      return Tag
        .findOne()
        .then(result => {
          return chai.request(app).put(`/api/tags/${result.id}`).send(updateTag);
        })
        .then(result => {
          res = result;
          expect(res).to.have.status(200);
  ...
  describe('DELETE endpoints', () => {
    it('should delete a tag with given id', () => {
      let tagHolder;
      return Tag
        .findOne()
        .then(tag => {
        ...
        .then(res => {
          ...
          return Tag.findById(tagHolder.id);
        })
        .then(folder => {
    ...
    // it('should delete all associated tags with it from notes', () => {
    //   let tagHolder;
    //   return Tag
    //     .findOne()
    //     .then(tag => {
